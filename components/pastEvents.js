...
const { getEvent } = require("../data-manager/pastEvents-data-methods");
...
let _pastevents = {};
_pastevents.process = async cmd => {
    return await transact(await validate(await check(cmd)));
};
const past_events_service = async (req, res) => {
    console.log(223)
    let pastevents = Object.create(_pastevents);
    let tmp = {};
    tmp.$$ = {};
    // get the req.body
    ...
    try {
        let y = await pastevents.process(tmp);
        // if success, send status = 200 along with result
        if (y.proceed) res.status(y.statusCode).json({
            status:true,
            result:y.result});
...
module.exports = { past_events_service, _pastevents };
